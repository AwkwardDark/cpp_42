	...
	for (size_t i = 0; i < str.length(); i++)
	{
		...
		if (sep == std::string::npos)
		{
			...
			if (!is_number(token) && !is_operator(token))
				throw std::runtime_error("Error: bad operators or numbers => " + token);
			rpn(token);
			break;
		}
		else
...
void RPN::rpn(std::string token)
{
	...
	if (is_operator(token))
	{
		...
		_rpnStack.pop();
		if (_rpnStack.size() == 0)
			throw std::runtime_error("Error: syntax is not corect => " + token);
		n2s << _rpnStack.top();
		_rpnStack.pop();
		n1s >> n1;
		n2s >> n2;
...
bool RPN::is_number(std::string str)
{
	...
	if (nb >= 10)
		...
		throw std::runtime_error("Number must be positive => " + str);
	return (true);
}
